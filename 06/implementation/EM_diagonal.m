function [alpha, mu, Sigma, L] = EM_diagonal(X, M, alpha_0, mu_0, Sigma_0, max_iter)
%EM calculates expectation maximation when Sigma is a diagonal matrix
%  size(alpha) = M,1
%  size(mu) = M,size(X,2)
%  size(Sigma) = M,size(X,2)
...
for i = 1:max_iter    
    ...
    for m = 1:M
        ...
        Sigma(m,:) = 1/N_m(m) * sum(r_mn_temp .* (X - mu_temp).^2, 1);
